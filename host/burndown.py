...
# Trailing slice used for the fit, clamped into [1h, 24h].
FIT_LOOKBACK_MIN_S = 3600
FIT_LOOKBACK_MAX_S = 24 * 3600
...
def _downsample(points, limit, start, end):
    """Thin `points` to at most `limit`, evenly by time, keeping the newest."""
    if len(points) <= limit:
        return list(points)
...
def _headline(remaining, resets_label, status, burn_rate, allowance,
    ...
    """One sentence stating the situation. This is the product."""
    if status == STATUS_EXHAUSTED:
        return (f"Exhausted. Resets in {resets_label}." if resets_label
                else "Exhausted.")

    left = f"{remaining:.0f}% left"
    head = f"{left}, {resets_label} to reset." if resets_label else f"{left}."
    # An estimate off token history is worth saying out loud; a measured rate
    # should not be hedged.
    estimated = rate_source == "estimated"
    rate = f"about {_fmt_rate(burn_rate)}" if estimated else _fmt_rate(burn_rate)
    tail = " Based on your recent usage." if estimated else ""

    if status == STATUS_CRITICAL and exhausts_label and burn_rate:
        return (f"{head} At {rate}%/{unit} you run out "
                f"{exhausts_label}.{tail}")
    if burn_rate is not None and allowance is not None and status == STATUS_AHEAD:
        return (f"{head} Burning {rate}%/{unit} against a "
                f"{_fmt_rate(allowance)}%/{unit} budget.{tail}")
    if delta is not None and burn_rate is not None:
        return f"{head} On pace, {_points(delta)} to spare.{tail}"
    return f"{head} Collecting history for a forecast."
...
def compute(provider, pool, payload, *, now=None, points=DEFAULT_POINTS,
    ...
    recent = [p for p in series if p[0] >= now - lookback]
